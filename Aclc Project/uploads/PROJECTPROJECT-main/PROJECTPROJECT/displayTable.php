...
if(isset($_GET['searchbar'])){
    ...
    $records = $conn->query("SELECT * FROM `tablelistfingerprintenrolled` WHERE `name` LIKE '$searchthis'");
    $numOfRows = $records->num_rows;
    $pageVisible = min(1 + floor($numOfRows / 5), 5);
    //calculating the number of rows each pages.
    $pages = ceil($numOfRows / $rowsPerPage);
    ...
    // </script>';
    // exit();
    $result = $conn->query("SELECT * FROM `tablelistfingerprintenrolled` WHERE `name` LIKE '$searchthis' LIMIT $start,$rowsPerPage");
}
?>