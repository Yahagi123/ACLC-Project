...
$result = $conn->query("SELECT * FROM `tablelistauditlogs` WHERE `name` = 'Intruder'  ORDER BY `DATE` DESC LIMIT $start,$rowsPerPage");

if(isset($_GET['searchbar'])){
    $searchthis = "%".strval($_GET['searchbar'])."%";
    $records = $conn->query("SELECT * FROM `tablelistauditlogs` WHERE `name` LIKE '$searchthis'");
    $numOfRows = $records->num_rows;
    $pageVisible = min(1 + floor($numOfRows / 5), 5);
    //calculating the number of rows each pages.
    $pages = ceil($numOfRows / $rowsPerPage);
