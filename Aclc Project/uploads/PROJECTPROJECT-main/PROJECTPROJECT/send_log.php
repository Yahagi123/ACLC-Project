...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $stmt = $pdo->prepare("UPDATE `fingerprint_status` SET `status` = :status, `message` = :message WHERE `id` = 1");
        $stmt->execute(['status' => $status, 'message' => $message]);

        echo json_encode(['status' => 'success', 'message' => 'Log updated successfully']);
    } elseif ($_SERVER['REQUEST_METHOD'] === 'GET') {
        // Fetch the latest status for the frontend
        $stmt = $pdo->query("SELECT `status` FROM `fingerprint_status` WHERE `id` = 1");
